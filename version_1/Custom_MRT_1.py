...
def Excutemosaic(mosfile, mosaicfile, bands):
    ...
    mrtbindir = os.path.dirname(os.path.dirname(pwdpath)) + os.path.sep + 'resource' + os.path.sep + \
                    'MRT' + os.path.sep + 'bin' 
...
# 利用MRT处理各分组HDF文件
def MRTProcess(indir, tempdir, localfiles, bands, 
               projection, resolution, ranges, typesuffixs, loginfofile, error):
    
    t = 1
    localregisterfiles = [[] for i in range(len(typesuffixs))]
    for i in range(len(localfiles)):
        
        aux.Writelog(loginfofile, '开始生成' + str(t).strip() + '分组HDF数据的镶嵌配置文件...', 1)
        ...
        if os.path.exists(mosfname):
            aux.Writelog(loginfofile, str(t).strip() + '分组HDF数据的镶嵌配置文件输出完成!', 1)
        else:
            aux.Writelog(loginfofile, str(t).strip() + '分组HDF数据的镶嵌配置文件输出失败!', 0)
            t += 1
            continue
        
        aux.Writelog(loginfofile, '开始镶嵌' + str(t).strip() + '分组HDF数据...', 1)
        mosaicfile = tempdir + os.path.sep + 'mosaic_' + str(t).strip() + '.hdf'
        ret = Excutemosaic(mosfname, mosaicfile, bands)
        if ret == 1 :
            aux.Writelog(loginfofile, str(t).strip() + '分组HDF数据的镶嵌完成!', 1)
        else:
            aux.Writelog(loginfofile, str(t).strip() + '分组HDF数据的镶嵌失败!', 0)
            t += 1
            continue
        
        aux.Writelog(loginfofile, '开始生成' + str(t).strip() + '分组HDF数据校正的配置文件...', 1)
        ...
        if ret == 1 :
            aux.Writelog(loginfofile, str(t).strip() + '分组HDF数据校正配置文件输出完成!', 1)
        else:
            aux.Writelog(loginfofile, str(t).strip() + '分组HDF数据校正配置文件输出失败', 10)
            t += 1
            continue
        
        aux.Writelog(loginfofile, '开始校正' + str(t).strip() + '分组HDF数据...', 1)
        files = Excuteresample(mosaicfile, regfname, registerfile, typesuffixs)
        if type(files) == list and len(typesuffixs) == len(files):
            aux.Writelog(loginfofile, str(t).strip() + '分组HDF数据校正完成!', 1)
            for j in range(len(files)):
                localregisterfiles[j].append(files[j])            
        else:
            aux.Writelog(loginfofile, str(t).strip() + '分组HDF数据校正失败!', 0)
            t += 1
            continue
...
# 获得各分块HDF文件
def GetSlice_HDF(indir, product_type, error):
    '获得各分块HDF文件'
